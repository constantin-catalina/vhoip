"""
scripts/run_ablations_c6b_components.py
Orchestrator for C6b component ablation experiments.

Runs 2 ablation variants on folds 0-4:
  1. w/o learnable temperature scaling    (fixed temperature = 1.0)
  2. w/o improved MLP projection head     (original 2-layer MLP, no dropout/L2 norm)

Usage:
    python scripts/run_ablations_c6b_components.py --config configs/mphoi72.yaml --folds 5
"""
...
ABLATIONS = [
    {
        "name": "w/o_LearnableTemp",
        "overrides": ["model.learnable_temp=false"],
        "folds": [1, 2, 3, 4],
    },
    {
        "name": "w/o_ImprovedMLP",
        "overrides": ["model.use_simple_mlp=true"],
    },
]


def parse_args():
    parser = argparse.ArgumentParser(description="C6b component ablation study orchestrator")
    ...
    parser.add_argument("--csv", type=str, default="ablation_c6b_components_results.csv")
...
def main():
    ...
    results = []
    total_runs = sum(len(a.get("folds", range(args.folds))) for a in ABLATIONS)
    run_idx = 0

    for ablation in ABLATIONS:
        exp_name = f"c6_{ablation['name']}"
        fold_list = ablation.get("folds", list(range(args.folds)))
        print(f"\n>>> Ablation: {exp_name} (folds: {fold_list})")
