...
Orchestrator for C6 ablation experiments on MPHOI-72.

Runs each ablation variant on folds 0-4, logs to W&B under c6_<name>,
extracts full metrics (F1@10/25/50 + FSUM) from checkpoints, and produces
a summary table + CSV.

Usage:
    python scripts/run_ablations.py --config configs/mphoi72.yaml --folds 5
"""
...
ABLATIONS = [
    ...
    {
        "name": "w/o_LearnPrompts",
        "overrides": ["model.use_learnable_prompts=false"],
    },
    {
        "name": "w/o_L_PromptReg",
        "overrides": ["training.lambda4=0"],
    },
    {
        "name": "w/o_GeoBranch",
        "overrides": ["model.disable_geo_branch=true"],
    },
    {
        "name": "w/o_L_MI",
        "overrides": ["training.lambda2=0"],
    },
    {
        "name": "w/o_L_Cos",
...
def parse_args():
    ...
    parser.add_argument("--csv", type=str, default="ablation_results.csv")
...
def print_summary(results):
    ...
    print("=" * 80)
    print(f"{'Ablation':<20} {'Fold':>5} {'F1@10':>8} {'F1@25':>8} {'F1@50':>8} {'FSUM':>8}")
    print("-" * 80)
    for r in results:
        print(
            f"{r['ablation']:<20} {r['fold']:>5} "
    ...
    print("=" * 80)
    print(f"{'Ablation':<20} {'F1@10':>12} {'F1@25':>12} {'F1@50':>12} {'FSUM':>12}")
    print("-" * 80)
